    ...
    <section id="pricing" className="max-w-6xl mx-auto px-4 pb-20">
      <div className="bg-white border border-slate-200 rounded-2xl p-8 shadow-sm">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          <div>
            <h2 className="text-2xl font-semibold text-slate-900">Simple pricing that grows with you</h2>
            <p className="text-slate-600 mt-2">
              Start free, upgrade when you&apos;re ready to scale your freelance business.
            </p>
          </div>
          <div>
        ...
        <div className="grid md:grid-cols-3 gap-6 mt-8">
          {[
            ...
            { name: 'Solo Pro', price: '$12', note: 'Unlimited clients & invoices' },
            { name: 'Studio', price: '$24', note: 'Team seats + advanced analytics' },
          ].map((plan) => (
            <div key={plan.name} className="border border-slate-200 rounded-xl p-6">
              <h3 className="text-lg font-semibold text-slate-900">{plan.name}</h3>
              <p className="text-3xl font-semibold text-slate-900 mt-3">{plan.price}</p>
