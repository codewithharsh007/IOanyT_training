...
def write_line_items_sheet(wb, records):
    ...
    headers = [lbl for lbl, *_ in LINE_ITEM_COLUMNS]
    widths = [w for _, _, w, *_ in LINE_ITEM_COLUMNS]
    aligns = [a for _, _, _, _, a, _ in LINE_ITEM_COLUMNS]
    nfmts = [n for _, _, _, n, *_ in LINE_ITEM_COLUMNS]
    keys = [(k, src) for _, k, _, _, _, src in LINE_ITEM_COLUMNS]

    for ci, (lbl, width) in enumerate(zip(headers, widths), start=1):
    ...
    row_i = 2
    for rec in records:
        for li_idx, li in enumerate(rec.get("line_items", []), start=1):
            li["_line_num"] = li_idx
            fill = ALT_FILL if row_i % 2 == 0 else None
            for ci, ((key, src), nf, aln) in enumerate(
                zip(keys, nfmts, aligns), start=1
            ):
                val = li.get(key, "") if src == "item" else rec.get(key, "")
    ...
    if row_i > 2:
        tbl = Table(
            ...
            ref=f"A1:{get_column_letter(len(headers))}{row_i-1}",
        )
